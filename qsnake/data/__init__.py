...
def number2color(number):
    """
    Converts an element number to a color (for visualization purposes).
    ...
    if _colors is None:
        ...
        number2symbol(1)
        for n, name in enumerate(_symbols):
            _colors[n] = ((n+1.)/len(_symbols), 0, 0)
        _colors[symbol2number("H")] = (0, 0, 1)
        _colors[symbol2number("C")] = (1, 0, 0)
        _colors[symbol2number("Si")] = (0.5, 0.5, 0.5)
